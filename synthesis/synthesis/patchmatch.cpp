...
/* -------------------------------------------------------------------------
  ...
  Only slightly modified from the code for the original PatchMatch algorithm:
...
void check_im() {
		...
		if (system("identify test.jpg > null.txt") != 0) {	/* LAURA ADDED "test.png" BECAUSE SYSTEM RETURNS 1 WITHOUT AN IMAGE TO IDENTIFY */
